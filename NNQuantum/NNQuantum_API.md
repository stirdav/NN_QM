...
- **`FockLadder_execution.jl`** — thin orchestration only: `run_Fock_ladder`, which calls into `FockLadder_problem.jl` for settings/dynamics/dataset-creation/prediction and into `NNQuantum.jl` for training/saving/loading (including `generate_decom_basis` — that one's a plain call to `NNQuantum.jl`, not something defined here). This file holds no dataset-creation or NN-wrapper logic of its own. `Bspline_Fock_execution.jl`/`Bspline_Fock_problem.jl` are the same two-file shape, for the `:FL_1step_Bspline` protocol.
