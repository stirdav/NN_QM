...
**Status: written, not live.** Commented-out pseudocode in `HBAR-qubit_problem.jl`. One change from this text: the mechanical subsystem is named `:osc`, not `:mech` — aligned to `FUNCTION_MAPPING.md`, which uses `:osc` consistently throughout its operator mapping (§1, §2, §4); `:mech` was the first draft, corrected once `FUNCTION_MAPPING.md` was read in full.
...
**Status: written, not live.** Commented-out pseudocode in `HBAR-qubit_problem.jl`, exactly as sketched above (`H_JC`/`H0` built from `op(cs,:qubit,...)`/`op(cs,:osc,...)`). `FUNCTION_MAPPING.md` §2/§9, read after this text was written, independently confirms the gap is real (no rotating-frame builder anywhere in `QuantumDynamics`, not just a choice deferred by this plan) and lays out the same two options; the frame question itself is still unresolved.
...
**Status: partially live.** `π_pulse_shape` is ported verbatim (`old_version/ML_QM_library.jl:188-196`) as real, runnable code — unlike (c)/(d), it has no dependency on anything still-unbound, so there was no reason to leave it commented. `Ω(t)`/`Δ(t)`/the `add_time_dependence` wiring remain commented-out pseudocode, since they depend on `cs`/`H0` from (c)/(d). One correction from the text above: the coefficient passed for the `σz` term is `Δ(t)/2`, not `Δ(t)` — matches `old_version`'s `Ht.factors[2] = Δ(t)/2` and `FUNCTION_MAPPING.md` §3's own worked example, missed in this paragraph's original wording.
...
**Status: written, not live.** Commented-out pseudocode: `J = jump_operators(cs, [thermal_bath(:osc, γm, nthm)..., Dephasing(:qubit, κϕ), Decay(:qubit, κ)])` (again `:osc`, not `:mech`). `FUNCTION_MAPPING.md` §4 independently confirms all four factors match exactly.
